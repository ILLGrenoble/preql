...
@DisplayName("Integer parameter parser tests")
class IntegerParameterParserTest {
    private final IntegerParameterParser parser = new IntegerParameterParser();
    
    @Test
    @DisplayName("should successfully convert valid values")
    void valid() {
        assertThat(parser.parse(1L)).isInstanceOf(Integer.class);
        assertThat(parser.parse(1)).isInstanceOf(Integer.class);
        assertThat(parser.parse("1")).isInstanceOf(Integer.class);
    }
    ...
    void invalid() {
        assertThrows(InvalidQueryException.class, () -> parser.parse(Long.MAX_VALUE));
