...
@DisplayName("Boolean parameter parser tests")
class BooleanParameterParserTest {
    private final BooleanParameterParser parser = new BooleanParameterParser();

    @Test
    @DisplayName("should successfully convert valid values")
    void valid() {
        assertThat(parser.parse(true)).isInstanceOf(Boolean.class);
        assertThat(parser.parse("true")).isInstanceOf(Boolean.class);
    }
    ...
    void invalid() {
        ...
        assertThrows(InvalidQueryException.class, () -> parser.parse(1));
